...
discovered="$(
  curl --fail --silent --max-time 10 \
    ...
    --data '{"jsonrpc":"2.0","id":1,"method":"server/discover","params":{"_meta":{"io.modelcontextprotocol/protocolVersion":"2026-07-28","io.modelcontextprotocol/clientCapabilities":{},"io.modelcontextprotocol/clientInfo":{"name":"generated-workerd-check","version":"1.0.0"}}}}'
)"
uv run python -c \
  ...
  "${discovered}"
echo "contract[mcp].discover=${discovered}"
...
called="$(
  curl --fail --silent --max-time 10 \
    ...
    --data '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"_meta":{"io.modelcontextprotocol/protocolVersion":"2026-07-28","io.modelcontextprotocol/clientCapabilities":{},"io.modelcontextprotocol/clientInfo":{"name":"generated-workerd-check","version":"1.0.0"}},"name":"list_todos","arguments":{}}}'
)"
uv run python -c \
  'import json,sys; envelope=json.loads(sys.argv[1])["result"]; assert envelope["resultType"] == "complete"; result=envelope["structuredContent"]; assert result["subject"]; assert isinstance(result["todos"], list)' \
  "${called}"
